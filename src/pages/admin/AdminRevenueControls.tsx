  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await Promise.all([
        supabase
          .from("platform_settings")
          .upsert({ key: "revenue_controls", value: revenueSettings as unknown as Json, updated_by: user?.id }, { onConflict: "key" }),
        supabase
          .from("platform_settings")
          .upsert({ key: "ai_modules", value: { ai_fraud_detection, ai_affiliate_coaching, ai_product_matching, ai_commission_optimization } as unknown as Json, updated_by: user?.id }, { onConflict: "key" }),
        supabase
          .from("platform_settings")
          .upsert({ key: "micro_fees", value: { processing_buffer_fee: settings.processing_buffer_fee, withdrawal_fee_percent: settings.withdrawal_fee_percent, withdrawal_flat_fee: settings.withdrawal_flat_fee, verification_badge_fee: settings.verification_badge_fee } as unknown as Json, updated_by: user?.id }, { onConflict: "key" }),
        supabase
          .from("platform_settings")
          .upsert({ key: "vendor_tiers", value: { premium_vendor_fee_reduction: settings.premium_vendor_fee_reduction, premium_vendor_monthly_cost: settings.premium_vendor_monthly_cost, transparent_ledger_enabled } as unknown as Json, updated_by: user?.id }, { onConflict: "key" }),
      ]);
