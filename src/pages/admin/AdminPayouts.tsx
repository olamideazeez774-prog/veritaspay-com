...
export default function AdminPayouts() {
  ...
  const { data: payouts, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data: payoutData, error } = await supabase
        .from("payout_requests").select("*").order("created_at", { ascending: false });
  ...
  const getStatusBadge = (status: PayoutStatus) => {
    switch (status) {
      case "paid": return <Badge className="bg-success gap-1"><Check className="h-3 w-3" />Paid</Badge>;
      case "rejected": return <Badge variant="destructive" className="gap-1"><X className="h-3 w-3" />Rejected</Badge>;
      case "processing": return <Badge variant="secondary" className="gap-1"><Clock className="h-3 w-3" />Processing</Badge>;
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="relative flex-1 max-w-sm">
            ...
            <Input placeholder="Search by user or account..." value={search} onChange={(e) => setSearch(e.target.value)} className="pl-10" />
          </div>
        ...
        {isLoading ? (
          ...
          <EmptyState icon={Wallet} title="No payout requests" description={statusFilter !== "all" ? "No requests match your filter." : "No payout requests have been submitted."} />
        ) : (
          <>
            {/* Mobile cards */}
            <motion.div variants={staggerContainer} initial="initial" animate="animate" className="space-y-3 sm:hidden">
              {filteredPayouts.map((payout) => (
                <motion.div key={payout.id} variants={staggerItem} className="glass-card p-4 space-y-3">
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">{payout.bank_name} · {payout.account_name}</p>
                    </div>
                    <Button variant="outline" size="sm" onClick={() => { setSelectedPayout(payout); setAdminNotes(payout.admin_notes || ""); }}>
                      {payout.status === "pending" || payout.status === "processing" ? "Process" : "View"}
                    </Button>
                  </div>
                      ...
                      <motion.tr key={payout.id} variants={staggerItem}>
                        ...
                        <TableCell>
                          <div className="text-sm">
                            ...
                            <p className="text-muted-foreground">{payout.account_name} · ****{payout.account_number?.slice(-4)}</p>
                          </div>
                        </TableCell>
                        ...
                        <TableCell className="text-right">
                          <Button
                            variant={payout.status === "pending" || payout.status === "processing" ? "outline" : "ghost"}
                            size="sm"
                            onClick={() => { setSelectedPayout(payout); setAdminNotes(payout.admin_notes || ""); }}
                          >
                            {payout.status === "pending" || payout.status === "processing" ? "Process" : "View"}
                          </Button>
                        </TableCell>
        ...
        <Dialog open={!!selectedPayout} onOpenChange={() => setSelectedPayout(null)}>
          <DialogContent className="max-w-md">
            <DialogHeader>
              ...
              <DialogDescription>Review the details and update the payout status.</DialogDescription>
            </DialogHeader>
            {selectedPayout && (
              <div className="space-y-4 py-4">
                <div className="rounded-lg bg-muted p-4 space-y-2">
                  {[
                    ...
                    ["Name", selectedPayout.account_name],
                  ].map(([label, value]) => (
                ...
                <div className="space-y-2">
                  ...
                  <Textarea placeholder="Add notes..." value={adminNotes} onChange={(e) => setAdminNotes(e.target.value)} rows={3} />
                </div>
              </div>
            )}
            <DialogFooter className="gap-2 sm:gap-0">
              {selectedPayout?.status === "pending" || selectedPayout?.status === "processing" ? (
                <>
                  <Button variant="destructive" onClick={() => updatePayout.mutate({ payoutId: selectedPayout!.id, status: "rejected", notes: adminNotes })} disabled={updatePayout.isPending}>
                    <X className="mr-1 h-4 w-4" /> Reject
                  </Button>
                  <Button onClick={() => updatePayout.mutate({ payoutId: selectedPayout!.id, status: "paid", notes: adminNotes })} disabled={updatePayout.isPending}>
